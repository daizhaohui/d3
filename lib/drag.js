...
import PrisonPopulationChart from './common/prisonChart';

const draggableChart = Object.create(PrisonPopulationChart);
draggableChart.addDragBehavior = function addDrag() {
  ...
  const dragContainer = this.container.append('rect').classed('bar-container', true).attr('width', this.svg.node().getBBox().width)
...
(async(enabled) => {
  if (!enabled) return;
  await draggableChart.init();
  draggableChart.addDragBehavior();
})(true);