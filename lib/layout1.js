...
import westerosChart from './common/westerosChart';
...
westerosChart.histogram = function histogram(_data) {
  const data = _data.data.map(d =>
      Object.assign(d, {
        death: (d.death.season * 100) + d.death.episode
      }))
...
westerosChart.stack = function Stack({
  data
}, isStream = false) {
  const episodesPerSeason = 10;
  const totalSeasons = 6;
...
westerosChart.force = function Force(_data) {
  const nodes = uniques(
    ...
    .map(d => ({
      id: d,
      total: _data.filter(e => e.Source === d).length
    }));
